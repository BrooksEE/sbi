...
typedef struct
{
	...
	USERFUNCID _userfid;
    VARIABLE _r[REG_SIZE];
    VARIABLE *_t; // ptr to global variables
 ...
 */
byte getval(const byte type, const byte val, void* t )
{
...
/*
	...
	Useful for user functions
		Return:
			...
			is not a variable
 */
unsigned int setval(const byte type, const byte num, const byte val, void* t) 
{
    SBITHREAD *thread=(SBITHREAD*)t;
    if (type==_varid)
    {
...
sbi_error_t sbi_begin(void *rt) {
	...
	SBITHREAD* thread = _sbi_createthread(0, (sbi_runtime_t*)rt);
...
sbi_error_t _sbi_step_internal(SBITHREAD* thread, sbi_runtime_t* rt)
{
	...
	switch (rd)
	{
		case _istr_assign:
            var1t= _getfch();
			var1 = _getfch();
			setval(var1t,var1,_getfch(), thread);
			break;
		case _istr_move:
            var1t = _getfch();
			var1 = _getfch();
        ...
        case _istr_gte:
            ...
            var3t = _getfch();
            var3 = _getfch();
            byte v1=getval(var1t,var1, thread); 
            byte v2=getval(var2t,var2, thread);
            byte val = rd == _istr_add ? v1+v2 :
                       rd == _istr_sub ? v1-v2 :
                       rd == _istr_mul ? v1*v2 :
                       rd == _istr_div ? v1/v2 :
        ...
        case _istr_tob:
            {
                var1t=_getfch();
                var1=_getfch();
                byte val = getval( var1t, var1, thread);
		...
		case _istr_debug:
			var1t = _getfch();
			_debug(getval(var1t, _getfch(), thread));
			break;
		case _istr_error:
			var1t = _getfch();
			_error(getval(var1t, _getfch(), thread));
			return 5;
			break;
		case _istr_sint:
			var1t = _getfch();
			thread->_userfid=getval(var1t, _getfch(), thread);
			break;
		case _istr_int:
            {
        ...
        case _istr_wait:
            {
                var1t = _getfch();
                var1 = _getfch();
                byte tId = getval(var1t,var1,thread);
